...
Da es sich bei den Variablen cut, color und clarity um nominalskalierte Daten handelt, kann mit diesen Variablen keine lineare Regression berechnet werden. Allerdings bin ich mir nicht im Klaren darüber, wie man die nominalskalierten Daten dennoch mit den intervallskalierten verrechnen kann.

Der Preis eines Diamanten wird am meisten vom Karatgehalt bestimmt, Größe, Länge und Breite haben aber auch einen Einfluss auf den Preis. Der Grund hierfür könnte sein, dass Größe, Länge und Breite einen großen Einfluss auf den Karatgehalt haben. Dies wird im Folgenden untersucht.

```{r}
summary(lm(carat~x, data=diamonds))
```
Bei dieser Berechnung zeigt sich, dass R² einen Wert von 0.95 aufweist, was bedeutet, dass sehr viel Varianz erklärt wird. Größe hat also einen sehr großen Einfluss auf Karat.

```{r}
summary(lm(carat~y, data=diamonds))
```
Auch die Variable y (width) erklärt viel Varianz bei Karat (R²=0.91), was ein klein wenig geringer ist, als bei x.

```{r}
summary(lm(carat~z, data=diamonds))
```
Auch bei z (depth) liegt der Wert von R² bei 0.91.

Die Variablen length, width und depth haben einen großen Einfluss auf Karat, und, da Karat einen großen Einfluss auf den Preis hat, auch einen großen Einfluss auf den Preis.
...
Bei dieser Hausaufgabe habe ich explorativ gearbeitet, das heißt, dass ich keiner richtigen Fragestellung gefolgt bin, sondern ein wenig "herumexperimentiert" habe. Wäre ich konfirmativ vorgegangen, hätte ich gezielter Tests anwenden können, um meine Fragestellungen zu beantworten, um meine Hypothese zu verifizieren (oder auch zu verwerfen).

# Lizenz
Dieses Werk dient ausschließlich Prüfungszwecken.
